...
const OverViewWindow = ({ onClose, data, name="", description="", assignedToUsers=[], initialSkills=[] }) => {
//   const [scores, setScores] = useState(data.childrenItems.map((item) => 0));
  ...
  // Handle quiz submission and sending the score back to the parent window
  const handleSubmit = async () => {

    await request.send('PATCH', '/api/user/me', answears);
    // Calculate the score and perform any necessary logic
    // ...

    // Send the score back to the parent window
    // window.opener.postMessage(score, window.origin);
  ...
  return (
    <Box sx={{ maxHeight: '450px', overflow: 'auto', borderRadius: "5px 5px 5px", border: "2px solid rgb(175, 203, 247)", boxShadow: "0px 0px 5px ", mb: 2 }} >
      ...
      </Box>
      {
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 2, borderBottom: "1px solid #ccc" }}>
        <Typography variant="h5">
            <span style={{ fontSize: '1.2em', fontWeight: 'bold', color: "rgb(51, 102, 120)" }}>Assigned to:</span> {assignedToUsers.map((user, index) => (
            <Typography variant="body1" component="span" sx={{ color: "gray" }}>
                {user.name}{index !== assignedToUsers.length - 1 && ", "}
